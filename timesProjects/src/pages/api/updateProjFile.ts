	...
	if (req.method == "POST") {
		...
		const imageHandler = async (data: any) => {
			let dataTypeLength = 0,
				dataType = "";
			if (
				data.slice(0, "data:image/jpg;base64,".length) ==
				"data:image/jpg;base64,"
			) {
				dataTypeLength = "data:image/jpg;base64,".length;
				dataType = "jpg";
			} else if (
				data.slice(0, "data:image/jpeg;base64,".length) ==
				"data:image/jpeg;base64,"
			) {
				dataTypeLength = "data:image/jpeg;base64,".length;
				dataType = "jpeg";
			} else if (
				data.slice(0, "data:image/png;base64,".length) ==
			...
			if (dataTypeLength > 0) {
				...
				try {
					let status = await writeFile(
						`public/${req.body["projectId"]}${today.format(
							"YYYYMMDD_HHmmss"
						)}.${dataType}`,
						bin,
						"binary"
					...
					projRepo.EditProject(req.body["projectId"], {
						projectImageURL: `/${req.body["projectId"]}${today.format(
							"YYYYMMDD_HHmmss"
						)}.${dataType}`,
					});
			...
			projRepo.getProjectById(req.body["projectId"]).then(async (record) => {
				if (record.length > 0) {
					...
					if (req.body["projectWhitePaper"]) {
						if (
							req.body["projectWhitePaper"].slice(
								0,
								"data:application/pdf;base64,".length
							) == "data:application/pdf;base64,"
						) {
							fileToWrite = req.body["projectWhitePaper"].slice(
								"data:application/pdf;base64,".length
							);
							let bin = Base64.atob(fileToWrite);
							// Your code to handle binary data
							try {
								let status = await writeFile(
									`public/whitePaper${req.body["projectId"]}${today.format(
										"YYYYMMDD_HHmmss"
									)}.pdf`,
									bin,
									"binary"
								...
								projRepo.EditProject(req.body["projectId"], {
									projectWhitePaperURL: `/whitePaper${
										req.body["projectId"]
									}${today.format("YYYYMMDD_HHmmss")}.pdf`,
								});
